...
fn process_one(env_args: &[String]) -> anyhow::Result<cmp_app::curl::CmdOptConf> {
    cmp_app::curl::parse_cmdopts("prog", env_args)
}
...
fn criterion_benchmark(c: &mut Criterion<CyclesPerByte>) {
    ...
    let v = vec![
        ...
        "http://url1.com"];
    let env_args: Vec<String> = v.iter().map(|s| s.to_string()).collect();
    let env_args = &env_args;
    //
    let result_conf = cmp_app::curl::CmdOptConf {
