...
fn process_one(env_args: &[&str]) -> anyhow::Result<cmp_argh::curl::CmdOptConf> {
    cmp_argh::curl::parse_cmdopts("prog", env_args)
}

fn criterion_benchmark(c: &mut Criterion<CyclesPerByte>) {
    #[rustfmt::skip]
    let env_args = vec![
        "-a", "--connect-timeout", "50", "--ftp-pasv", "--httptwo",
        "--max-time", "100", "--no-alpn", "-N",
        "--socksfive-gssapi-service", "name1", "-y", "1000", "--sslvthree",
        "http://url1.com"];
    let env_args = &env_args;
    //
    let result_conf = cmp_argh::curl::CmdOptConf {
    ...
    //
    c.bench_function("cmp_argh::curl::", |b| b.iter(|| {
