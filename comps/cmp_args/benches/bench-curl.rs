...
fn process_one(env_args: &[&str]) -> anyhow::Result<cmp_args::curl::CmdOptConf> {
    cmp_args::curl::parse_cmdopts("prog", env_args)
}
...
fn criterion_benchmark(c: &mut Criterion<CyclesPerByte>) {
    ...
    //
    let result_conf = cmp_args::curl::CmdOptConf {
    ...
    //
    c.bench_function("cmp_args::curl::", |b| b.iter(|| {
