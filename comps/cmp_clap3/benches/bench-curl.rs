...
fn process_one(env_args: &[&str]) -> anyhow::Result<cmp_clap3::curl::CmdOptConf> {
    cmp_clap3::curl::parse_cmdopts("prog", env_args)
}
...
fn criterion_benchmark(c: &mut Criterion<CyclesPerByte>) {
    ...
    //
    let result_conf = cmp_clap3::curl::CmdOptConf {
    ...
    //
    c.bench_function("cmp_clap3::curl::", |b| {
        b.iter(|| {
