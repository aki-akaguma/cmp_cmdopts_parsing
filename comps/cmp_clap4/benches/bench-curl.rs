...
fn process_one(env_args: &[&str]) -> anyhow::Result<cmp_clap4::curl::CmdOptConf> {
    cmp_clap4::curl::parse_cmdopts("prog", env_args)
}
...
fn criterion_benchmark(c: &mut Criterion<CyclesPerByte>) {
    ...
    //
    let result_conf = cmp_clap4::curl::CmdOptConf {
    ...
    //
    c.bench_function("cmp_clap4::curl::", |b| {
        b.iter(|| {
