...
pub fn parse_cmdopts(_program: &str, env_args: Vec<&str>) -> anyhow::Result<CmdOptConf> {
    ...
    args.cnt_verbose = matches.get_count("verbose") as usize;
    args.opt_speed = match matches.get_one::<f32>("speed") {
        Some(x) => *x,
        None => 0.0,
    };
    /*
    args.opt_speed = match matches.get_one::<String>("speed") {
        Some(x) => x.parse::<f32>().unwrap(),
        None => 0.0,
    };
    */
    args.opt_color = match matches.get_one::<String>("color") {
