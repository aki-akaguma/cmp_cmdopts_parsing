...
fn process_one(env_args: &[&str]) -> anyhow::Result<cmp_flood_tide::curl::CmdOptConf> {
    cmp_flood_tide::curl::parse_cmdopts("prog", env_args)
}
...
fn criterion_benchmark(c: &mut Criterion<CyclesPerByte>) {
    ...
    //
    c.bench_function("cmp_flood_tide::curl::", |b| b.iter(|| {
