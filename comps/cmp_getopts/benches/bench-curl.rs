...
fn process_one(env_args: &[&str]) -> anyhow::Result<cmp_getopts::curl::CmdOptConf> {
    cmp_getopts::curl::parse_cmdopts("prog", env_args)
}
...
fn criterion_benchmark(c: &mut Criterion<CyclesPerByte>) {
    ...
    //
    let result_conf = cmp_getopts::curl::CmdOptConf {
    ...
    //
    c.bench_function("cmp_getopts::curl::", |b| b.iter(|| {
