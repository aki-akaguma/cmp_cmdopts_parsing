...
fn process_one(env_args: &[&str]) -> anyhow::Result<cmp_gumdrop::curl::CmdOptConf> {
    cmp_gumdrop::curl::parse_cmdopts("prog", env_args)
}
...
fn criterion_benchmark(c: &mut Criterion<CyclesPerByte>) {
    ...
    //
    let result_conf = cmp_gumdrop::curl::CmdOptConf {
    ...
    //
    c.bench_function("cmp_gumdrop::curl::", |b| b.iter(|| {
