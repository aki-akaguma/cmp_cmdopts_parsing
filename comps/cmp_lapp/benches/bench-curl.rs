...
fn process_one(env_args: &[&str]) -> anyhow::Result<cmp_lapp::curl::CmdOptConf> {
    cmp_lapp::curl::parse_cmdopts_str("prog", env_args)
}
...
fn criterion_benchmark(c: &mut Criterion<CyclesPerByte>) {
    ...
    //
    let result_conf = cmp_lapp::curl::CmdOptConf {
    ...
    //
    c.bench_function("cmp_lapp::curl::", |b| b.iter(|| {
