...
fn process_one(env_args: &[&str]) -> anyhow::Result<cmp_pure_rust::curl::CmdOptConf> {
    cmp_pure_rust::curl::parse_cmdopts("prog", env_args)
}
...
fn criterion_benchmark(c: &mut Criterion<CyclesPerByte>) {
    ...
    //
    let result_conf = cmp_pure_rust::curl::CmdOptConf {
    ...
    //
    c.bench_function("cmp_pure_rust::curl::", |b| b.iter(|| {
