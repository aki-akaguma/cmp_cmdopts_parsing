...
fn process_one(env_args: &[&str]) -> anyhow::Result<cmp_rustop::curl::CmdOptConf> {
    cmp_rustop::curl::parse_cmdopts("prog", env_args.to_vec())
}
...
fn criterion_benchmark(c: &mut Criterion<CyclesPerByte>) {
    ...
    //
    let result_conf = cmp_rustop::curl::CmdOptConf {
    ...
    //
    c.bench_function("cmp_rustop::curl::", |b| b.iter(|| {
