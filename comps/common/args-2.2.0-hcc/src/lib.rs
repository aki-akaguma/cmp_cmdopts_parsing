...
//!
//! use args_hcc::{Args,ArgsError};
//! use args_hcc::validations::{Order,OrderValidation};
//!
...
use getopts::{Fail, HasArg, Matches, Occur, Options};
...
impl Args {
    ...
    pub fn parse<C: IntoIterator>(&mut self, raw_args: C) -> Result<Matches, ArgsError>
    where
        C::Item: AsRef<OsStr>,
    {
        ...
        for opt_name in &self.opt_names {
            ...
            let value = opt.parse(&matches).unwrap_or_else(|| "".to_string());
            if !value.is_empty() {
    ...
    /// # Failures
    /// Fails if any errors are encountered during parsing.
    pub fn parse_from_cli(&mut self) -> Result<Matches, ArgsError> {
        ...
        T: FromStr,
    {
        ...
        self.value_of::<T>(opt_name).and_then(|value| {
            for validation in validations {
                ...
                if validation.is_invalid(&value) {
                    return Err(validation.error(&value));
                }
            }
    ...
    pub fn value_of<T: FromStr>(&self, opt_name: &str) -> Result<T, ArgsError> {
        self.values
            .get(opt_name)
            .ok_or_else(|| ArgsError::new(opt_name, "does not have a value"))
            .and_then(|value_string| {
                T::from_str(value_string).map_err(|_| {
                    ArgsError::new(opt_name, &format!("unable to parse '{}'", value_string))
                })
            })
    }
                ...
                values_str
                    ...
                    .map(|value| {
                        T::from_str(value).map_err(|_| {
                            ArgsError::new(opt_name, &format!("unable to parse '{}'", value))
                        })
                    })
                    .collect()
            })
    ...
    fn register_opt(&mut self, opt: Box<dyn Opt>) {
        if !self.opt_names.contains(&opt.name()) {
            ...
            self.opt_names.push(opt.name());
            self.opts.insert(opt.name(), opt);
        } else {
