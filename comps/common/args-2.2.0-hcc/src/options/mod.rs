...
macro_rules! unsupported {
    ( $str:expr ) => {
        panic!("{} is not supported yet", $str)
    };
}
...
pub fn new(
    ...
    if has_arg == HasArg::Maybe {
        unsupported!("HasArg::Maybe");
    }

    if occur != Occur::Multi {
        Box::new(Single::new(
            short_name, long_name, desc, hint, has_arg, occur, default,
        ))
    } else {
...
impl Multi {
    fn new(short_name: &str, long_name: &str, desc: &str, hint: &str) -> Self {
        Multi {
...
impl Opt for Multi {
    ...
    fn parse(&self, matches: &Matches) -> Option<String> {
        ...
        if strs.is_empty() {
            ...
            Some(strs.join(SEPARATOR))
        }
    }

    fn register(&self, options: &mut Options) {
        options.optmulti(&self.short_name, &self.long_name, &self.desc, &self.hint);
    }
...
impl Single {
    fn new(
        ...
        a_has_arg: HasArg,
        occur: Occur,
        a_default: Option<String>,
    ) -> Self {
        // If there is a default occurence becomes optional
        let c_occur = if a_default.is_some() {
            Occur::Optional
        } else {
            occur
        };
